...
- **豁免**：管理员可用 `/豁免 QQ号` 或 `/豁免 @某人` 豁免账号（不再检测并清除违规记录；**若该用户处于禁言状态自动解除禁言**），`/解除豁免`（或 `/取消豁免`）从豁免清单移除并恢复检测；也可在面板配置页增删
...
| 配置项 | 说明 | 面板操作 |
| ---- | ---- | ---- |
| `exempt_users` | 豁免 QQ 列表 | 添加/删除（`/豁免` `/解除豁免` 命令也会写入） |
...
| /豁免 QQ号 或 /豁免 @某人 | 豁免某用户：不再检测、清除违规记录，若被禁言自动解除（仅管理员） |
| /解除豁免 QQ号 或 /解除豁免 @某人 | 从豁免清单移除该用户，恢复检测（仅管理员） |
| /取消豁免 | 同 /解除豁免 |
