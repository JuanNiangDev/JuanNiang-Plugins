...
- **黑色地带**：命中 `words/black.txt`（来源 [campus-ad-detection-words/样本.md](https://github.com/excniesNIED/campus-ad-detection-words/blob/main/样本.md) 抽取的无歧义话术，如贷款强提、假通知群、0元购、流量卡、修图日结等）或 `words/cn_advertisement.txt` 时送 LLM 高危复核（防止"插入U盘""一台独立主机"这类关键词误触），确认违规后按三级惩罚处罚；LLM 不可用/异常时退回关键词直接处罚
- **灰色地带 LLM 审查**：命中 `words/all.txt`（校园卡、考研、群名片、加群、加微信、辱骂等语义模糊词）时**异步**送 Bot 自身 LLM 审查（`jn.llm`，复用 Bot Provider 配置，不阻塞消息流程），LLM 返回 JSON 判定违规类型（广告/敏感）后按三级惩罚追罚，判定正常则放行
- **敏感违规**：色情 / 政治 / 脏话 / 辱骂词库命中送 LLM 高危复核（兜底同黑色地带），确认后按三级惩罚处罚
...
两级制度均引入 LLM 复核：命中关键词后通过 `jn.llm.chat_async` 异步调用 Bot 当前启用的文本模型 Provider（模型/采样参数/密钥全部复用 Bot 配置，插件不接触密钥），同一用户同一时刻只有一个在途审查，同一条消息 10 分钟内不重复审查。

- **灰色地带**：常规审查提示词（`LLM_SYSTEM_PROMPT`），中性判定、拿不准倾向放行
- **敏感/黑名单高危词**：高危复核提示词（`BLACK_LLM_SYSTEM_PROMPT`），提示"命中高危关键词、语义上极可能违规，但不要被关键词匹配误导"（如"插入U盘"命中"插入"、"一台独立主机"命中"台独"这类正常语境应判放行）；LLM 确认违规后按三级惩罚处罚，LLM 不可用/异常时退回关键词直接处罚
- LLM 返回 JSON：`{"violation": "ad|sensitive|none", "reason": "..."}`；`violation` 为 `ad`/`sensitive` 时按对应类别触发三级惩罚
- 系统提示词内置在 `main.lua`（`LLM_CRITERIA` 公共判定标准 + `LLM_SYSTEM_PROMPT` / `BLACK_LLM_SYSTEM_PROMPT` 两套提示词），可自行调整
- 面板配置 `llm_review_enabled` 关闭后：敏感/黑名单词直接处罚、灰色词直接放行（不审查）
...
| `llm_review_enabled` | 关键词 AI 审查开关 | 开关；关闭后敏感/黑名单词直接处罚、灰色词直接放行 |
...
| 灰色地带（LLM 审查） | `words/all.txt` | [campus-ad-detection-words](https://github.com/excniesNIED/campus-ad-detection-words)（校园卡/考研/群名片/加群/辱骂等灰色词） |
...
| llm_review_enabled | bool | 关键词 AI 审查开关 | true |
